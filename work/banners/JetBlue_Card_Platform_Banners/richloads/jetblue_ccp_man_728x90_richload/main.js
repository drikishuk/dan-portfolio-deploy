...
function animate() {
  ...
  tl.addLabel('start', 0)
    ...
    .to(['#copy1', '#copy2', '#copy3'], 1, { autoAlpha: 1, y: '-=100', ease: "expo.out" }, '+=0')
    .to(['#endframe', '#pink-bar-1'], 0.5, { x: '+=192', ease: "expo.out" }, '+=2')
    .to(['#man', '#man-2', '#bkg', '#copy1'], 0.5, { x: '+=165',  ease: "expo.out" }, '-=0.5')
    .to(['#copy2'], 0.5, { x: '+=155', y: '-=13', ease: "none.none" }, '-=0.5')
    .to(['#copy3'], 0.5, { x: '+=58', y: '+=13', ease: "none.none" }, '-=0.5')
    .to(['#endframe_headline', '#terms'], 0.5, { autoAlpha: 1 }, '-=0.5')

    tl.to(['#man', '#man-2'], 9, { scale: 1.1, ease: Power1.easeInOut }, 0)
    tl.to(['#bkg'], 9, { scale: 1.2, ease: Power1.easeInOut }, 0)
}
